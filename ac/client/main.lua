...
AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
    if SuperStars_AC.AntiPedRevive then
        local _target, _distance = GetClosestPlayer()
        if _target ~= -1 then
            local _tid = GetPlayerServerId(_target)
            local _ped = PlayerPedId()
            local _pcoords = GetEntityCoords(_ped)
            local distance = #(SuperStars_AC.HospitalCoords - vector3(_pcoords))
            if distance < 50 then
...
AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checkifneargarage", function()
    if SuperStars_AC.AntiVehicleSpawn then
        local _pcoords = GetEntityCoords(PlayerPedId())
        local isneargarage = false
        for k,v in pairs(SuperStars_AC.GarageList) do
...
if SuperStars_AC.Enable then
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            local _ped = PlayerPedId()
            local _pid = PlayerId()
            if SuperStars_AC.GodModeProtection then
            ...
            Citizen.Wait(200)
            if SuperStars_AC.MainAnticheat then
                Citizen.Wait(50)
                if SuperStars_AC.AntiRagdoll then
                    if not CanPedRagdoll(_ped) then
                        if not IsPedInAnyVehicle(_ped, true) and not IsEntityDead(_ped) then
                            local closestVehicle, _distance = GetClosestVehicle()
                ...
                if SuperStars_AC.AntiInvisible then
                ...
                if SuperStars_AC.AntiRadar then
                ...
                if SuperStars_AC.AntiExplosiveBullets then
                ...
                Citizen.Wait(100)
                if SuperStars_AC.AntiFlyandVehicleBelowLimits then
                    ...
                    if tonumber(aboveground) > 25 then
                        if not IsPedInAnyVehicle(_ped, false) and not IsPedInParachuteFreeFall(_ped) and not IsPedFalling(_ped) then
                            ...
                            for height = 1, 100000 do -- Code Inspired in Qalle-TPM (https://github.com/qalle-git/esx_marker)
                                local _pcoords = GetEntityCoords(_ped)
            ...
            Citizen.Wait(200)
            if SuperStars_AC.AntiBoomDamage then
                SetEntityProofs(_ped, false, true, true, false, false, false, false, false)
            end
            Citizen.Wait(200)
            if SuperStars_AC.PlayerProtection then
            ...
            Citizen.Wait(200)
            if SuperStars_AC.AntiBlacklistedWeapons then
                if not bypassweapon then
                    for _,Weapon in ipairs(SuperStars_AC.BlacklistedWeapons) do
            ...
            Citizen.Wait(200)
            if SuperStars_AC.AntiThermalVision then
                if GetUsingseethrough() then
                    sendinfotoserver("Ue53dCG6hctHvrOaJB5Q", "thermalvision") -- BAN (THERMAL VISION)
                end
            end
            Citizen.Wait(200)
            if SuperStars_AC.AntiNightVision then
                if GetUsingnightvision() then
                    sendinfotoserver("Ue53dCG6hctHvrOaJB5Q", "nightvision") -- BAN (NIGHT VISION)
                end
            end
            Citizen.Wait(200)
            if SuperStars_AC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
            ...
            Citizen.Wait(200)
            if SuperStars_AC.AntiLicenseClears then
            ...
            Citizen.Wait(200)
            if SuperStars_AC.DisableVehicleWeapons then
            ...
            Citizen.Wait(200)
            local V = {"/e", "/f", "/d"}
            if SuperStars_AC.AntiKeyboardNativeInjections then
            ...
            Citizen.Wait(200)
            if SuperStars_AC.AntiCheatEngine then
            ...
            Citizen.Wait(200)
            if SuperStars_AC.AntiCInjection then
                for _,cmd in ipairs(GetRegisteredCommands()) do
                    if inTable(SuperStars_AC.BlackListedCMD, cmd.name) then
                        sendinfotoserver("Ue53dCG6hctHvrOaJB5Q", "commandinjection") -- BAN (COMMAND INJECTION)
                    end
                end
            end
            Citizen.Wait(200)
            if SuperStars_AC.AntiMenyoo then
                if IsPlayerCamControlDisabled() ~= false then
                    sendinfotoserver("Ue53dCG6hctHvrOaJB5Q", "menyoo") -- BAN (MENYOO)
                end
            end
            Citizen.Wait(200)
            if SuperStars_AC.AntiGiveArmour then
    ...
    Citizen.CreateThread(function() -- Props for Badger Anticheat for this (Modified and optimized)
        while SuperStars_AC.AntiNoclip do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
    ...
    Citizen.CreateThread(function()
        while SuperStars_AC.AntiBlips do
            Citizen.Wait(1000)
            local _pid = PlayerId()
    ...
    Citizen.CreateThread(function()
        while SuperStars_AC.AntiInfiniteAmmo do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedArmed(_ped, 6) then
                ...
                if SuperStars_AC.AntiDamageModifier then
    ...
    Citizen.CreateThread(function()
        while SuperStars_AC.AntiVehicleModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if SuperStars_AC.AntiFlyandVehicleBelowLimits then
                    ...
                    if tonumber(_vehcoords.z) < 10 then
                        for height = 1, 10000 do -- Code Inspired in Qalle-TPM (https://github.com/qalle-git/esx_marker)
    ...
    Citizen.CreateThread(function()
        while SuperStars_AC.AntiSuperJump do
            Citizen.Wait(810)
            local _ped = PlayerPedId()
    ...
    AddEventHandler('ZRQA3nmMqUBOIiKwH4I5:clearpeds', function()
        if SuperStars_AC.ClearPedsAfterDetection then
    ...
    AddEventHandler('gameEventTriggered', function (name, args)
        if SuperStars_AC.AntiSuicide then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearprops", function()
        if SuperStars_AC.ClearObjectsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearvehicles", function()
        if SuperStars_AC.ClearVehiclesAfterDetection then
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        if SuperStars_AC.DeleteBrokenCars then
    ...
    if SuperStars_AC.AntiResourceStartorStop then
    ...
    SuperStars_ACGetVehicles = function()
        local vehicles = {}
    ...
    SuperStars_ACGetClosestVehicle = function(coords)
        local vehicles        = SuperStars_ACGetVehicles()
...
RegisterNetEvent("super:screenshot")
AddEventHandler("super:screenshot", function(target)
    exports["discord-screenshot"]:requestCustomClientScreenshotUploadToDiscord(
        target,
        "https://discord.com/api/webhooks/934835691697868830/L0AKRfsg1z7q1CFMapLkdNlgrGPzUFZrsOJATCO8oa-qesuQm1KcBYTWkKLvTAviDCUb",
        {
            encoding = "png",
            quality = 1
        },
        {
            username = "SuperStar ScreenShot",
            avatar_url = "https://media.discordapp.net/attachments/934896669630287892/934943021475180594/Proiect_nou_27.png?width=671&height=671",
            content = ".",
            embeds = {
                {
                    color = 16771584,
                    author = {
                        name = ".",
                        icon_url = "https://media.discordapp.net/attachments/934896669630287892/934943021475180594/Proiect_nou_27.png?width=671&height=671"
                    },
                    title = "."
                }
            }
